...
module.exports = function (src, filePath, jestConfig) {
  ...
  if (parts.script && parts.script.src) {
    tsSrcPath = join(filePath, '..', parts.script.src)
    parts.script.content = fs.readFileSync(tsSrcPath, 'utf8')
  }

  const result = processScript(parts.script, vueJestConfig, tsSrcPath || filePath)
  ...
  const map = generateSourceMap(script, '', tsSrcPath || filePath, scriptSrc, inputMap)
