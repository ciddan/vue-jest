...
module.exports = function (src, filePath, jestConfig) {
  ...
  if (parts.script && parts.script.src) {
    srcFilePath = join(filePath, '..', parts.script.src)
    parts.script.content = fs.readFileSync(srcFilePath, 'utf8')
  }

  const result = processScript(parts.script, vueJestConfig, srcFilePath || filePath)
  ...
  const map = generateSourceMap(script, '', srcFilePath || filePath, scriptSrc, inputMap)
